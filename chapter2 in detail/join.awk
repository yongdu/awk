# join - join file1 file2 on the fisrt field
# input: two sorted files, tab-separated fileds
# output:natural join of lines with common first field
BEGIN { 
    OFS = seq ="\t"
    file2 = ARGV[2]
    ARGV[2] = "" #read file1 implcitly, file2  explicitly
...

{
    ...
    if (prefix($0) == prefix(gp[1])) # 1st attributes in filed1
...
function getgroup(){# put equal prefix group into gp[1..ng]
    if(getone(file2,gp,1 <=0)) # end of file
        return 0
    for (ng = 2; getone(file2, gp,ng); ng++)
...
function getone(f, gp, n){# get next line in gp[n]
    if (eofstat <= 0) # eof or error has occured
        return 0
...
function prefix(s) {return substr(s, 1, index(s,seq)-1)}
function suffix(s) {return substr(s, index(s,seq)+1)}